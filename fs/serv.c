...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((err = openfile_lookup(envid, req->req_fileid, &op)) < 0)
		return err;
	ssize_t count;
	req->req_n = MIN(PGSIZE, req->req_n);
	if ((count = file_read(op->o_file, ret->ret_buf, req->req_n, op->o_fd->fd_offset)) < 0)
		return count;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	ssize_t count;
	req->req_n = MIN(PGSIZE, req->req_n);
	if ((count = file_write(op->o_file, req->req_buf, req->req_n, op->o_fd->fd_offset)) < 0)
		return count;
	op->o_fd->fd_offset += count;
	return count;
}
